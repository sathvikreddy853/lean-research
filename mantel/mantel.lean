...
lemma neighborFinsets_disjoint_for_edge
  ...
  intro w huw hvw
  have guw : G.Adj u w := (mem_neighborFinset_iff_adj (G := G)).mp huw
  have gvw : G.Adj v w := (mem_neighborFinset_iff_adj (G := G)).mp hvw
  exact (no_common_neighbor_of_edge (G := G) htri huv) ⟨guw, gvw⟩
...
deg(u) + deg(v) ≤ |V|.
-/
lemma degree_add_degree_le_card_of_vertex_set
  (htri : TriangleFree (G := G)) {u v : V} (huv : G.Adj u v) :
  G.degree u + G.degree v ≤ Fintype.card V := by
  have hdisj :
      Disjoint (G.neighborFinset u) (G.neighborFinset v) :=
    neighborFinsets_disjoint_for_edge (G := G) htri huv

  rw [← card_neightFinset_eq_degree (G := G) u,
      ← card_neightFinset_eq_degree (G := G) v]

  rw [← Finset.card_union_of_disjoint hdisj]

  exact card_le_univ (G.neighborFinset u ∪ G.neighborFinset v)
...
/--
Sum of squared degrees bound.

By summing the inequality deg(u) + deg(v) ≤ |V| over all edges,
we obtain a global bound on the sum of squared degrees.

Mathematically:
∑_{v ∈ V} deg(v)^2 ≤ |V| · |E|.
-/
lemma sum_sq_degrees_le_card_edges_mul_card_vertex_set
